...
export default class Crypto {
  ...
  static decryptEak(readerKey, encryptedAk) {
    ...
    let eak = this.b64decode(encodedEak.split('.')[0])
    let nonce = this.b64decode(encodedEak.split('.')[1])
  ...
  static decryptRecord(encrypted, accessKey) {
    ...
    for (let key in encrypted.data) {
      if (encrypted.data.hasOwnProperty(key)) {
        ...
        let edk = this.b64decode(cipher[0])
        let edkN = this.b64decode(cipher[1])
        let ef = this.b64decode(cipher[2])
        let efN = this.b64decode(cipher[3])
