...
export default class Client {
   ...
   * the provided EAK and popupate the cache.
   *
   * @returns {Promise<string>}
   */
   ...
   *
   * @param {Record} record Record instance with plaintext data for encryption
   * @param {object} eak    Encrypted access key instance
   *
   * @returns {Promise<Record>}
   */
  async encrypt(type, data, plain = {}, eak) {
    ...
    let Document = class extends Signable {
      constructor(doc) {
        super()
    ...
    let dataDocument = new Document(data)
    let signature = await this.sign(dataDocument)
  ...
  static generateKeypair() {
    ...
    return new KeyPair(
      Crypto.b64encode(keypair.publicKey),
      Crypto.b64encode(keypair.privateKey)
    )
  }
