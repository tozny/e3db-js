  ...
  if (client._authToken === null) {
    return fetch(client.config.apiUrl + '/v1/auth/token', {
      ...
      .then(token => {
        ...
        client._authTokenTimeout = setTimeout(() => {
          client._authToken = null
        }, 10 * 60 * 1000)
...
async function oauthFetch(client, ...params) {
  ...
  let token = await refreshToken(client)
...
export default class Client {
   ...
   */
  close() {
    clearTimeout(this._authTokenTimeout)
  }
